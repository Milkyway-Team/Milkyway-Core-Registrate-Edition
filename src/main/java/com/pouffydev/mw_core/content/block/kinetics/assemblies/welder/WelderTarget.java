...
public class WelderTarget {
    static final WelderTarget NO_TARGET = new WelderTarget();
    static final WelderTarget PROCESSING_TARGET = new WelderTarget(true);
    
    float baseAngle;
    float lowerArmAngle;
    float upperArmAngle;
    float headAngle;
    private WelderTarget() {
        lowerArmAngle = -45;
        upperArmAngle = 0;
        headAngle = 0;
    }
    private WelderTarget(boolean processing) {
        lowerArmAngle = -45;
        upperArmAngle = 11.25f;
        headAngle = -45;
    }
    
    public WelderTarget(BlockPos armPos, Vec3 pointTarget, Direction clawFacing, boolean ceiling) {
        ...
        headPos = VecHelper.rotate(headPos.add(0, b, 0), beta + 180, Direction.Axis.X);
        headPos = VecHelper.rotate(headPos.add(0, a, 0), alpha - 90, Direction.Axis.X);
        headPos = VecHelper.rotate(headPos, baseAngle, Direction.Axis.Y);
        headPos = VecHelper.rotate(headPos, ceiling ? 180 : 0, Direction.Axis.X);
