...
public class WeldingBehaviour extends BeltProcessingBehaviour {
    ...
    protected void spawnParticles() {
        ...
        if (mode == Mode.BELT)
            particleItems.forEach(stack -> makeWeldingParticleEffect(VecHelper.getCenterOf(worldPosition.below(2))
                    .add(0, 8 / 16f, 0), stack));
        if (mode == Mode.WORLD)
            particleItems.forEach(stack -> makeWeldingParticleEffect(VecHelper.getCenterOf(worldPosition.below(1))
                    .add(0, -1 / 4f, 0), stack));

        particleItems.clear();
    }
    public void makeWeldingParticleEffect(Vec3 pos, ItemStack stack) {
        makeWeldingParticleEffect(pos, stack, specifics.getParticleAmount());
    }
    public void makeWeldingParticleEffect(Vec3 pos, ItemStack stack, int amount) {
        Level level = getWorld();
        if (level == null || !level.isClientSide)
            return;
